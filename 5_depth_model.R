...
jags.Depth <- MixSIAR::run_model(run="very short", mix, source, discr, model_filename)

output_options <- list(summary_save = TRUE,                                                 # save summary statistics as txt file
                       summary_name = "output/Environment_Depth Model/summary_statistics",  # path for summary stats
                       sup_post = TRUE,                                                     # suppress posterior density plot, default is FALSE
                       plot_post_save_pdf = TRUE,                                           # save posterior density plot
                       plot_post_name = "output/Environment_Depth Model/posterior_density", # path for posterior density plots
                       sup_pairs = TRUE,                                                    # suppress pairs plot
                       plot_pairs_save_pdf = TRUE,                                          # save pairs plot
                       plot_pairs_name = "output/Environment_Depth Model/pairs_plot",       # path for pairs plot
                       sup_xy = TRUE,                                                       # suppress x/y trace plot
                       plot_xy_save_pdf = TRUE,                                             # save x/y trace plot
                       plot_xy_name = "output/Environment_Depth Model/xy_plot",             # path for x/y trace plot
                       ...
                       diag_name = "output/Environment_Depth Model/diagnostics",            # path for diagnostics
...
# call function to save the ouputs of jags.Depth following the predefined options
# input 1 Yes when prompted to safely save output in working directory
MixSIAR::output_JAGS(jags.Depth, mix, source, output_options) # safe to ignore warnings - NDK

# save jags.Depth object for further analysis
saveRDS(jags.Depth,"output/Environment_Depth Model/jags.Depth.rds")

...
jags.1 <- readRDS("output/jags.1.rds")

# Directly access MCMC draws for SCAM mixing proportion for each individual 
# assumes SCAM is first mixing element
p.scam <- jags.1$BUGSoutput$sims.list$p.ind[,,1]
p.sppa <- jags.1$BUGSoutput$sims.list$p.ind[,,2]
...
ggplot(bgb_biomass, aes(x = seed_year, y = prop_sppa, color = env_treatment, shape = Depth)) +
  ...
  ylim(0,0.018) +
  facet_wrap(~ env_treatment) +  # Separate panels for each treatment
  labs(title = expression(Dry~Weight~Density~of~SPPA~(g/cm^{3})),  # Title with superscript
       x = "Year", y = expression(Weight~Density~of~SPPA~(g/cm^{3}))) +
  theme_bw()  # Set a black and white theme (optional)
...
saveRDS(bgb_biomass,"output/bgb_biomass.rds")

...
mix.filename <-"data/training_TOP.csv"
...
mix[[8]] <- load_mix_data(filename=mix.filename,
                          iso_names=c("d13C"),
                          factors=c("salinity","elev_bi"),
                          fac_random=c(FALSE,FALSE),
                          fac_nested=c(FALSE,FALSE),
                          cont_effects="seed_year")
mix[[9]] <- load_mix_data(filename=mix.filename,
                          iso_names=c("d13C"),
                          factors=c("salinity","elev_bi"),
                          fac_random=c(FALSE,FALSE),
                          fac_nested=c(FALSE,FALSE),
                          cont_effects=NULL)
mix[[10]] <- load_mix_data(filename=mix.filename,
                          iso_names=c("d13C"),
                          factors=c("gt"),
                          fac_random=c(TRUE),
                          fac_nested=c(FALSE),
...
names(jags.mod) <- c("NULL","Environment","Seed Year",
                     "Environment:Seed Year","Environment/GT:Seed Year",
                     "Environment/GT","GT","Sal/Elev:Seed Year",
                     "Sal/Elev","GT:Seed Year")
comparison.table <- compare_models(jags.mod,loo = F)
...
saveRDS(jags.mod,"output/Model Compare/jags.compare.rds")

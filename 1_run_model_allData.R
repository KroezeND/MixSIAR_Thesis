...
# Outputs: "Full/jags.EnvGT.rds",
# outputs/Full/model_X:
...
holdout_TOP <- read.csv("data/holdout_avg_TOP.csv")
holdout_MID <- read.csv("data/holdout_avg_MID.csv")
holdout_BTM <- read.csv("data/holdout_avg_BTM.csv")

full_TOP <- rbind(training_TOP,holdout_TOP)
full_MID <- rbind(training_MID,holdout_MID)
full_BTM <- rbind(training_BTM,holdout_BTM)

write.csv(full_TOP,"data/full_TOP.csv",row.names = FALSE)
write.csv(full_MID,"data/full_MID.csv",row.names = FALSE)
write.csv(full_BTM,"data/full_BTM.csv", row.names = FALSE)
...
mix[[1]] <- MixSIAR::load_mix_data(filename="data/full_TOP.csv",
                                   iso_names=c("d13C"),
                                   factors=c("env_treatment","gt"),
                                   fac_random=c(FALSE,TRUE),
                                   fac_nested=c(FALSE,FALSE),
                                   cont_effects="seed_year")
mix[[2]] <- MixSIAR::load_mix_data(filename="data/full_MID.csv",
                                   iso_names=c("d13C"),
                                   factors=c("env_treatment","gt"),
                                   fac_random=c(FALSE,TRUE),
                                   fac_nested=c(FALSE,FALSE),
                                   cont_effects="seed_year")
mix[[3]] <- MixSIAR::load_mix_data(filename="data/full_BTM.csv",
                                   iso_names=c("d13C"),
                                   factors=c("env_treatment","gt"),
                                   fac_random=c(FALSE,TRUE),
                                   fac_nested=c(FALSE,FALSE),
...
for(mod in 1:n.mod){
  ...
  dir.create(file.path('output/Full', subDir), showWarnings = FALSE)
  
  # isospace plot
  MixSIAR::plot_data(filename=paste0("output/Full/", subDir, "/isospace_plot_", mod),
                     plot_save_pdf=TRUE,
                     plot_save_png=FALSE,
                     mix[[mod]], source[[mod]], discr[[mod]])
  
  # Define model structure and write JAGS model file
  model_filename <- paste0("output/Full/", subDir, "/MixSIAR_model_", mod, ".txt")
  resid_err <- TRUE
  process_err <- TRUE
  ...
  # Run the JAGS model
  jags.EnvGT[[mod]] <- MixSIAR::run_model(run="test", mix[[mod]], source[[mod]], discr[[mod]], model_filename, alpha.prior=1)
  
  output_options <- list(
    summary_save = TRUE,                                                        # save summary statistics as txt file
    summary_name = paste0("output/Full/", subDir, "/summary_statistics", mod),  # path for summary stats
    sup_post = TRUE,                                                            # suppress posterior density plot, default is FALSE
    plot_post_save_pdf = TRUE,                                                  # save posterior density plot
    plot_post_name = paste0("output/Full/", subDir, "/posterior_density", mod), # path for posterior density plots
    sup_pairs = TRUE,                                                           # suppress pairs plot
    plot_pairs_save_pdf = TRUE,                                                 # save pairs plot
    plot_pairs_name = paste0("output/Full/", subDir, "/pairs_plot", mod),       # path for pairs plot
    sup_xy = TRUE,                                                              # suppress x/y trace plot
    plot_xy_save_pdf = TRUE,                                                    # save x/y trace plot
    plot_xy_name = paste0("output/Full/", subDir, "/xy_plot", mod),             # path for x/y trace plot
    ...
    diag_name = paste0("output/Full/", subDir, "/diagnostics", mod),            # path for diagnostics
...
saveRDS(jags.EnvGT,"output/Full/jags.EnvGT.rds")
